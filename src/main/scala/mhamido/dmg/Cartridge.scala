...
final case class Cartridge(
  ...
  def info: String = {
    ...
    | Publisher:       ${publisher}
    | Version:         ${versions}
    | Entry:           0x${entry.toHexString}
    | SGB Flag:        ${sgb}
    | Type:            ${kind}
    | ROM Size:        ${romSize}% 4d MiB
    | RAM Size:        ${ramSize}% 4d KiB
    | Dest. Code:      ${if (destination == 0) "JAPAN" else "OVERSEAS"}
...
object Cartridge {
  def apply(path: String): Try[Cartridge] =
    for {
      bytes <- Try { Files.readAllBytes(Paths.get(path)) }
      header <- Header(bytes)
    } yield Cartridge(path, bytes, header)
